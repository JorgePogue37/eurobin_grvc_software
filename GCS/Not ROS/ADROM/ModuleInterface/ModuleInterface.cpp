...
{
	DATA_PACKET_MODULE_MSG dataPacketModuleRequest;
	int bytesSent = 0;
	int errorCode = 0;
	
	// Set the fields of the request data packet
	dataPacketModuleRequest.header[0] = 'R';
	dataPacketModuleRequest.header[1] = 'E';
	dataPacketModuleRequest.header[2] = 'Q';
	dataPacketModuleRequest.moduleID = this->moduleID;
	dataPacketModuleRequest.msgCode = _requestCode;
	dataPacketModuleRequest.msgOption = _requestOption;
	dataPacketModuleRequest.userBufferSize = 0;
	
	// Send the request data packet
	bytesSent = sendto(this->socketPublisher, (char*)&dataPacketModuleRequest, sizeof(DATA_PACKET_MODULE_MSG), 0, (struct sockaddr*)&addrHost, sizeof(struct sockaddr));
...
/*
 * Send a message to the module, including a pointer to optional user data to be sent.
 */
int ModuleInterface::sendModuleRequestParams(uint8_t _msgCode, uint8_t _msgOption, uint8_t * _bufferData, size_t _bufferDataSize)
{
	...
	// Set the fields of the request data packet
	dataPacketMsg.header[0] = 'R';
	dataPacketMsg.header[1] = 'E';
	dataPacketMsg.header[2] = 'Q';
