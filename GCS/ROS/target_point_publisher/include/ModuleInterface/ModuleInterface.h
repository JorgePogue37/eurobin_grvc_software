...
public:
	 ...
	 */
	int sendMessage(uint8_t _msgCode, uint8_t _msgOption);
	
	/*
	 * Return 1 if a request was received from manager, and zero in other case. 
	 */
	 ...
	 * Return the request code when received.
	 */
	void getRequestParams(int & _requestCode, int & _requestOption, float * pos, float * quat);
